        ...
        switch (fd.getFieldType()) {
            ...
            case INTEGER: {
                if (fd.getDataLen() == 1) {
                    ...
                    throw new Exception("Unsupported integer length: " 
                            + fd.getDataLen() + " for field: " + fd.getFieldName());
                }
                break;
            }
            case DECIMAL: {
            ...
            case REAL: {
                if (fd.getDataLen() == 4) {
                    ...
                    throw new Exception("Unsupported real length: " 
                            + fd.getDataLen() + " for field: " + fd.getFieldName());
                }
                break;
            }
            ...
            case RECORD: {
                ...
                for (int i = 0; i < recordAccessor.getNumFields(); i++) {
                    dataLen += calculateFieldSize(recordAccessor.getFieldDefinition(i),
                                                  recordAccessor.getChildRecordAccessor(i),
