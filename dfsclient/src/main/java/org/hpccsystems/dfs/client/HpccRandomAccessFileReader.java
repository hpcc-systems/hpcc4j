...
public class HpccRandomAccessFileReader<T> implements Iterator<T>
{
    ...
    public T next()
    {
        ...
        if (numRemainingRecords == 0)
        {
            throw new java.util.NoSuchElementException("No records have been requested");
        }
        ...
        if (requestedRecordOffsets.size() == 0)
        {
            ...
            Runnable fetchRequestTask = new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    {
                        throw new java.util.NoSuchElementException("Error while performing block fetch request: " + e.getMessage());
                    }
                }
            };
