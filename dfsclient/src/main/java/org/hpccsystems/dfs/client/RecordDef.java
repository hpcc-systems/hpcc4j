...
public class RecordDef implements Serializable
{
     ...
     */
    public static FieldDef parseFieldDef(DefToken first, Iterator<DefToken> toks_iter, HashMap<String, TypeDef> type_dict)
     ...
     */
    public static TypeDef parseTypeDef(DefToken first, Iterator<DefToken> toks_iter, HashMap<String, TypeDef> type_dict)
            throws UnusableDataDefinitionException
    {
        if (first.getToken() != JsonToken.START_OBJECT)
        {
        ...
        while (toks_iter.hasNext() && curr.getToken() != JsonToken.END_OBJECT)
        {
            ...
            else if (fieldListName.equals(curr.getName()))
            {
                ...
                curr = toks_iter.next();
                while (curr.getToken() != JsonToken.END_ARRAY && toks_iter.hasNext())
                {
    ...
    static public RecordDef fromJsonDef(String defThor, ColumnPruner cp) throws UnusableDataDefinitionException
    {
        ...
        // create record def
        FieldDef root = new FieldDef("root", FieldType.RECORD, "none", len, type_id == type_record, HpccSrcType.UNKNOWN,
