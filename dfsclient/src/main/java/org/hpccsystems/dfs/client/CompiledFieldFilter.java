...
public class CompiledFieldFilter
{
    ...
    public boolean matchesRange(Object inclusiveBegin, Object inclusiveEnd)
    {
        ...
        // 1 if inclusive end is greater than lowerValue, 0 if equal2
        int endCompare = compare(inclusiveEnd, lowerValue);

        if (beginCompare >= lowerComparisonType && endCompare >= lowerComparisonType)
        {
            return true;
        }
        ...
        // 1 if inclusive ned is greater than lowerValue, 0 if equal
        endCompare = compare(inclusiveEnd, upperValue);

        if (beginCompare >= upperComparisonType && endCompare >= upperComparisonType)
        {
            return true;
        }
