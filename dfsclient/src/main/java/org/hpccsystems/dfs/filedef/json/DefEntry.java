...
public abstract class DefEntry implements Serializable
{
    ...
    //
    protected static int getTokenCount(DefToken[] toks, int startPos) throws UnusableDataDefinitionException
    {
        int curr_level = toks[startPos].getLevel();
        int tokenCount = 1;
        while (startPos + tokenCount < toks.length && curr_level < toks[startPos + tokenCount].getLevel())
        {
            tokenCount++;
        }
        tokenCount++; // count the close
        if (startPos + tokenCount >= toks.length || !DefToken.pairedTypes(toks[startPos].getToken(), toks[startPos + tokenCount - 1].getToken()))
        {
