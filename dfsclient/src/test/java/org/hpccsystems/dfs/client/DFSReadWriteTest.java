...
public class DFSReadWriteTest extends BaseRemoteTest
{
    ...
    private static final String[] datasets           = { "~benchmark::integer::20kb", "~unit_test::all_types::thor", "~unit_test::all_types::json",
    ...
    public void recordSamplingRateTest() throws Exception
    {
        ...
        System.out.println("Expected sample count: " + expectedSampleCount + ", Actual sample count: " + records.size() + ", Percentage difference: "
                + percentageDiff + "%");

        // Allowing 10% variance due to sampling
        assertTrue("Percentage difference in sampled records is too high: " + percentageDiff + " should be less than 10%", percentageDiff < 10.0);
    }
    ...
    public void readResumeTest() throws Exception
    {
        ...
        {
            HPCCRecordBuilder recordBuilder = new HPCCRecordBuilder(file.getProjectedRecordDefinition());
            HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[resumeFilePart.get(i)], originalRD,
                    recordBuilder, 1000000, -1, true, readSizeKB, resumeInfo.get(i));
    ...
    public void nullWriteTest() throws Exception
    {
        ...
        List<HPCCRecord> records = readFile(file, connTO, false);
        assertEquals("Record count mismatch for dataset:" + fname + ". got: " + records.size() + " expected:" + expectedCounts[1], expectedCounts[1],
                records.size());
        ...
        assertEquals("Record count mismatch for dataset:" + copyFileName + ". got: " + records.size() + " expected:" + expectedCounts[1],
                expectedCounts[1], records.size());
    ...
    public void numericOverflowTest() throws Exception
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            record.setField(15, new BigDecimal(BigInteger.valueOf(((Long) record.getField(15)).longValue()).and(UNSIGNED_LONG_MASK)));
    ...
    public void stringProcesingTests() throws Exception
    {
        ...
        fieldDefs[7] = new FieldDef("str8", FieldType.VAR_STRING, "FIXED_VAR_UNICODE", fixedStrLen, true, false, HpccSrcType.UTF16LE,
                new FieldDef[0]);
        fieldDefs[8] = new FieldDef("str9", FieldType.VAR_STRING, "FIXED_VAR_STRING", fixedStrLen, true, false, HpccSrcType.SINGLE_BYTE_CHAR,
                new FieldDef[0]);
    ...
    public void resumeFileReadTest() throws Exception
    {
        ...
        for (int i = 0; i < fileParts.length; i++)
        {
            ...
            HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[i], originalRD, recordBuilder, -1, -1, true,
                    -1, partitionResumeList.get(i));
    ...
    public void protocolVersionTest()
    {
        ...
        {
            assertTrue("Expected rowservice with version: " + remoteVersion.toString() + " to be using new protocol.",
                    fileReader.getInputStream().isUsingNewProtocol());
        }
        else
        {
            assertFalse("Expected rowservice with version: " + remoteVersion.toString() + " to be using old protocol.",
                    fileReader.getInputStream().isUsingNewProtocol());
        }
    }
    ...
    public void filePartReadRetryTest()
    {
        // Test basic retry functionality
        {
            ...
            try
            {
                ...
                HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[0], originalRD, recordBuilder,
                        HpccRemoteFileReader.DEFAULT_CONNECT_TIMEOUT_OPTION, 10, // limit to 10 records for test
                        true, DEFAULT_READ_SIZE_OPTION);
        ...
        // Test maximum retry limit
        {
            ...
            try
            {
                ...
                HPCCRecordBuilder recordBuilder = new HPCCRecordBuilder(readFile.getProjectedRecordDefinition());
                HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[0], originalRD, recordBuilder,
                        HpccRemoteFileReader.DEFAULT_CONNECT_TIMEOUT_OPTION, 10, true, DEFAULT_READ_SIZE_OPTION);
    ...
    public void readRetryOptionsPreservationTest()
    {
        ...
        try
        {
            ...
            HPCCRecordBuilder recordBuilder = new HPCCRecordBuilder(readFile.getProjectedRecordDefinition());
            HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[0], originalRD, recordBuilder,
                    HpccRemoteFileReader.DEFAULT_CONNECT_TIMEOUT_OPTION, NO_RECORD_LIMIT, true, DEFAULT_READ_SIZE_OPTION, null, 15000  // 15 second socket timeout
            );
            ...
            assertEquals("Initial string processing flags should be set", expectedStringFlags,
                    fileReader.getRecordReader().getStringProcessingFlags());
            assertEquals("Initial decimal processing option should be set", expectedUseDecimal,
            ...
            assertEquals("String processing flags should be preserved after retry", expectedStringFlags,
                    fileReader.getRecordReader().getStringProcessingFlags());
            assertEquals("Decimal processing option should be preserved after retry", expectedUseDecimal,
                    fileReader.getRecordReader().getUseDecimalForUnsigned8());
    ...
    public void invalidSignatureTest()
    {
...

        {
            ...
            try
            {
                ...
                HPCCWsDFUClient dfuClient = wsclient.getWsDFUClient();
                DFUCreateFileWrapper createResult = dfuClient.createFile(datasets[0] + "-copy123", this.thorClusterFileGroup, eclRecordDefn, 300,
                        false, DFUFileTypeWrapper.Flat, "");
                ...
                for (int partitionIndex = 0; partitionIndex < hpccPartitions.length; partitionIndex++)
                {
                    ...
                    fileWriter = new HPCCRemoteFileWriter<HPCCRecord>(hpccPartitions[partitionIndex], recordDef, recordAccessor,
    ...
    public List<HPCCRecord> readFile(HPCCFile file, Integer connectTimeoutMillis, boolean shouldForceTimeout, boolean useDecimalForUnsigned8)
            throws Exception
    {
    ...
    public List<HPCCRecord> readFile(HPCCFile file, Integer connectTimeoutMillis, boolean shouldForceTimeout, boolean useDecimalForUnsigned8,
            int stringProcessingFlags) throws Exception
    {
        if (file == null)
        {
        ...
        for (int i = 0; i < fileParts.length; i++)
        {
            try
            {
                HPCCRecordBuilder recordBuilder = new HPCCRecordBuilder(file.getProjectedRecordDefinition());
                HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[i], originalRD, recordBuilder,
                        RowServiceInputStream.DEFAULT_CONNECT_TIMEOUT_MILIS, NO_RECORD_LIMIT, true, DEFAULT_READ_SIZE_OPTION, null,
            ...
            catch (Exception e)
            {
                ...
                {
                    if (cause.getCause() instanceof java.net.SocketTimeoutException) System.out.println(
    ...
    private void writeFileAndReportAnyExceptions(List<HPCCRecord> records, String fileName, FieldDef recordDef, Integer connectTimeoutMs,
            boolean compressed) throws Exception
    {
        ...
        for (int partitionIndex = 0; partitionIndex < hpccPartitions.length; partitionIndex++)
        {
            ...
            {
                fileWriter = new HPCCRemoteFileWriter<HPCCRecord>(hpccPartitions[partitionIndex], recordDef, recordAccessor, compressionAlgorithm,
                        connectTimeoutMs);
                //wait a bit longer than the default timeout to ensure the override connect timeout
                //is being honoured
                if (connectTimeoutMs != null && connectTimeoutMs > RowServiceOutputStream.DEFAULT_CONNECT_TIMEOUT_MILIS + 1)
                {
