...
public class DFSIndexTest extends BaseRemoteTest {
    ...
    public void hpccTLKFilterTest() throws Exception {
        for (int i = 0; i < datasetNames.length; i++) {
            ...
            for (int j = 0; j < fileParts.length - 1; j++) {
                ...
                HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[j],
    ...
    public void tlkFilterExample() throws Exception {
        ...
        HPCCRecordBuilder recordBuilder = new HPCCRecordBuilder(file.getProjectedRecordDefinition());
        HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(matchedPart,
    ...
    public void biasedIntTest() throws Exception {
        ...
        for (int j = 0; j < fileParts.length; j++) {
            HPCCRecordBuilder recordBuilder = new HPCCRecordBuilder(file.getProjectedRecordDefinition());
            HpccRemoteFileReader<HPCCRecord> fileReader = new HpccRemoteFileReader<HPCCRecord>(fileParts[j], originalRD,
                    recordBuilder);
            while (fileReader.hasNext()) {
    ...
    private void createIndexableFile(String fileName, FieldDef recordDef, List<HPCCRecord> partitionRangeStart,
            List<HPCCRecord> partitionRangeEnd) {
        try {
            // ------------------------------------------------------------------------------
            ...
            for (int partitionIndex = 0; partitionIndex < hpccPartitions.length; partitionIndex++) {
                ...
                HPCCRemoteFileWriter<HPCCRecord> fileWriter = new HPCCRemoteFileWriter<HPCCRecord>(
