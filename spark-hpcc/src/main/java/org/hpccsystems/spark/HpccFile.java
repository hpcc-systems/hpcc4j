...
public class HpccFile extends org.hpccsystems.dfs.client.HPCCFile implements Serializable
{
    ...
    public void setRecordSamplingRate(double recSamplingRate) throws IllegalArgumentException
    {
        if (recSamplingRate < MIN_RECORD_SAMPLING_RATE || recSamplingRate > MAX_RECORD_SAMPLING_RATE)
        {
            throw new IllegalArgumentException(
                    "Record sampling rate must be between " + MIN_RECORD_SAMPLING_RATE + " and " + MAX_RECORD_SAMPLING_RATE);
        }
    ...
    public HpccRDD getRDD(SparkContext sc, List<Integer> filePartsToRead) throws HpccFileException
    {
        ...
        HpccRDD rdd = new HpccRDD(sc, filteredFileParts.toArray(new DataPartition[0]), this.getRecordDefinition(),
                this.getProjectedRecordDefinition(), this.getFileAccessExpirySecs(), this.recordLimit, this.recordSamplingRate,
                this.recordSamplingSeed);
        rdd.setTraceContext(parentTraceID, parentSpanID);
        return rdd;
    }
    ...
    public Dataset<Row> getDataframe(SparkSession session) throws HpccFileException
    {
        ...
        HpccRDD hpccRDD = new HpccRDD(session.sparkContext(), fp, originalRD, projectedRD, this.getFileAccessExpirySecs(), this.recordLimit,
                this.recordSamplingRate, this.recordSamplingSeed);
