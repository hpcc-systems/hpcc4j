...
public class DFUFileDetailInfo extends DFUFileDetail
{
     ...
     * @throws Exception
     */
    public EclInfo deduceFields() throws Exception
    {
        ...
        if (fileType == FileFormat.FLAT || fileType == FileFormat.KEYED)
        {
            ...
            if (getEcl() != null && !getEcl().isEmpty()) {
                ...
                EclInfo ei=new EclInfo(new DFURecordDefInfo(getColumns()));
        ...
        else if (fileType == FileFormat.XML)
        {
            if (hasEcl())
            {
                EclInfo ei= GetRecordFromECL(getEcl());
                ei.setFileType(fileType);
                return ei;
        ...
        else if (fileType == FileFormat.CSV)
        {
            ...
            if (!isSprayedCsv())
            {
                EclInfo ei= DFUFileDetailInfo.GetRecordFromECL(getEcl());
                ei.setFileType(fileType);
                return ei;
            }
            ...
            else if (getFirstline() != null && !getFirstline().isEmpty())
            {
                ...
                EclInfo ei= new EclInfo(new DFURecordDefInfo(fields));
                ei.setFileType(fileType);
                return ei;
            }
            else
...
//        boolean eclissprayed=false;
    ...
    private EclInfo addKeyInfo(EclInfo input) {
        ...
        for (Entry<String,DFURecordDefInfo> val:input.getRecordsets().entrySet()) {
            if (val.getKey().startsWith(EclInfo.UNNAMED)) {
