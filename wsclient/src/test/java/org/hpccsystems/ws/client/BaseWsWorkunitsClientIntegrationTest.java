...
public abstract class BaseWsWorkunitsClientIntegrationTest extends BaseRemoteTest
{
    ...
    public void testSearchQueries() throws Exception
    {
        ...
        List<QueryResultWrapper> resultsArr2 = wswuclient.searchQueries(QuerySetFilterType.Name, queryname, roxiecluster.getName(),
                roxiecluster.getName());
        if (resultsArr2 == null || resultsArr2.size() != 1)
        {
            Assert.fail("Should have returned one query for query named " + queryname);
        }
        List<QueryResultWrapper> resultsArr3 = wswuclient.searchQueries(QuerySetFilterType.Id, queryid, roxiecluster.getName(),
                roxiecluster.getName());
    ...
    public void testGetWorkunitSort() throws Exception
    {
        ...
        {
            Assert.fail("descending clusters in wrong order:" + result.get(0).getCluster() + " then " + result.get(result.size() - 1).getCluster());
        }
        //cluster ascending
        ...
        {
            Assert.fail("ascending clusters in wrong order:" + result.get(0).getCluster() + " then " + result.get(result.size() - 1).getCluster());
        }
        ...
        {
            Assert.fail("descending jobname in wrong order:" + result.get(0).getJobname() + " then " + result.get(result.size() - 1).getJobname());
        }
        //jobname ascending
        ...
        {
            Assert.fail("ascending jobname in wrong order:" + result.get(0).getJobname() + " then " + result.get(result.size() - 1).getJobname());
        }
        ...
        {
            Assert.fail("descending owner in wrong order:" + result.get(0).getOwner() + " then " + result.get(result.size() - 1).getOwner());
        }
        //owner ascending
        ...
        {
            Assert.fail("ascending owner in wrong order:" + result.get(0).getOwner() + " then " + result.get(result.size() - 1).getOwner());
        }
        ...
        {
            Assert.fail("descending state in wrong order:" + result.get(0).getState() + " then " + result.get(result.size() - 1).getState());
        }
        //state ascending
        ...
        {
            Assert.fail("ascending state in wrong order:" + result.get(0).getState() + " then " + result.get(result.size() - 1).getState());
        }
    }
    ...
    public void testAbortWU() throws Exception, ArrayOfEspExceptionWrapper, ArrayOfECLExceptionWrapper
    {
        ...
        if (!(WUState.ABORTED.toString().toLowerCase().equals(test.getState()) || WUState.ABORTING.toString().toLowerCase().equals(test.getState())))
        {
