...
public abstract class BaseWsAttributesClientIntegrationTest extends BaseRemoteTest
{
    ...
    {
        if (platform.getVersion().toString().startsWith(getHPCCVersion())) throw new Exception(
                platform.getIP() + " is no longer on version " + getHPCCVersion() + "! It is now on " + platform.getVersion().toString());
    }

    @Before
    public void setup() throws Exception
    {
        if (wsattport == null) wsattport = "8145";
    ...
    public void testWriteAttributes() throws Exception, ArrayOfEspExceptionWrapper
    {
        ...
        System.out.println(
                "Time to create " + testdir.listFiles().length + " attributes with checkout/checkin:" + (System.currentTimeMillis() - begin));
