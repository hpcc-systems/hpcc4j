...
public class RDFHPCCWsClient extends HPCCWsClient {
	...
	// (Resolving relative URIs and fragment IDs is done by prepending the base
	// URI to the relative URI/fragment.)
	// If there are no relative URIs in the source, this argument may safely be
	// null.
	// If the base is the empty string, then relative URIs will be retained in
	// the model.
	// This is typically unwise and will usually generate errors when writing
	// the model back out.
	private String targetRDFBaseURI = null;

	// Predefined values for lang are "RDF/XML", "N-TRIPLE", "TURTLE" (or "TTL")
	// and "N3".
	// null represents the default language, "RDF/XML". "RDF/XML-ABBREV" is a
	// synonym for "RDF/XML".
	...
	public void setTargetRDFLanguage(String targetRDFLanguage) {
		if (languages.contains(targetRDFLanguage)) {
			...
			Utils.println(System.out, "Only valid entries are: 'RDF/XML', 'N-TRIPLE', 'TURTLE' (or 'TTL') and 'N3'.",
					false, verbosemode);
		}
	...
	public boolean sprayHPCCFile() {
		...
		if (targetDestinationGroup != null) {
			success = sprayDefaultCSVHPCCFile(targetHPCCFilePath, targetHPCCFilePath, targetDestinationGroup,
					overwrite);
			success &= sprayCustomCSVHPCCFile(targetHPCCFilePath + "::namespaces", targetHPCCFilePath + "::namespaces",
					targetDestinationGroup, outputEscape, outputDelimiter, outputQuote, outputTerminator, overwrite,
			...
			if (setDropZoneInfo()) {
				...
				if (success = fioclient.createHPCCFile(targetHPCCFilePath, targetDropzoneNetAddres, overwrite)) {
					...
					if (hpccnsprefixmap != null) {
						if (fioclient.createHPCCFile(targetHPCCFilePath + "::namespaces", targetDropzoneNetAddres,
								true)) {
							nsMappingsCSVFormat = hashMapToCSV(hpccnsprefixmap, outputDelimiter, outputTerminator);
						} else {
							Utils.println(System.out, "Could not create target HPCC namespaces file.", false,
									verbosemode);
						}
					}

					// Remote HPCC file has been created, encode the data so it
					// can be uploaded as binary data.
					// byte [] encodedBytes =
					// encodeData(triplesCSVFormat.toString());
					// The data was being encoded, but latest test don't seem to
					// require encoding.
					// if (success =
					// writeHPCCFileData(triplesCSVFormat.toString().getBytes(),
					// baseurl, targetHPCCFilePath, targetDropzoneNetAddres,
					// uploadchunksize))
					if (success = fioclient.writeHPCCFileData(triplesCSVFormat.toString().getBytes(),
							targetHPCCFilePath, targetDropzoneNetAddres, false, 0, uploadchunksize)) {
						if (nsMappingsCSVFormat != null && nsMappingsCSVFormat.length() > 0) {
							if (!fioclient.writeHPCCFileData(nsMappingsCSVFormat.toString().getBytes(),
									targetHPCCFilePath + "::namespaces", targetDropzoneNetAddres, false, 0,
									uploadchunksize)) {
								Utils.println(System.out, "Could not write to HPCC namespaces file.", false,
										verbosemode);
							}
						}
					}
	...
	static public Model createModelFromFile(String targetrdflanguage, String targetrdflocation, String targetrdfbaseuri,
			boolean verbosemode) {
		...
		if (in == null) {
			...
			try {
				...
				Utils.println(System.out,
						"Error while loading model: Language Syntax error: " + e.getLocalizedMessage(), false,
						verbosemode);
	...
	static public Map<String, String> getNameSpaceMapping(Model themodel, List<String> additionalNSs,
			boolean verbosemode) {
		...
		if (themodel != null && !themodel.isClosed() && !themodel.isEmpty()) {
			...
			for (String namespace : nsPrefixMap.values()) {
				...
				Utils.println(System.out,
						"Namespace from mapping: " + namespace + "  -- Abbreviated: " + lhpccnsprefixmap.get(namespace),
						false, verbosemode);
			}
			...
			if (additionalNSs != null) {
				for (String namespace : additionalNSs) {
					lhpccnsprefixmap.put(namespace, HPCCNAMESPACE + nsindex++);
					Utils.println(System.out, "Additional namespace added: " + namespace + " -- Abbreviated: "
							+ lhpccnsprefixmap.get(namespace), false, verbosemode);
				}
			}
		}
					...
					if (!subject.isAnon()) {
						...
						if (subjectsNS != null && subjectShorthandNS == null) {
							if (subject.isURIResource()) {
								Utils.println(System.err, "Possible issue detected, URI without known NS: " + subject,
										true, verbosemode);
										...
										if (substring.length() > 0) {
											if (fixNSIssues) {
												...
												Utils.println(System.err,
														"WARNING MAKING FOLLOWING ASSUMPTION: NameSpace: "
																+ namespacekey + " LocalName: " + subjectName,
														true, verbosemode);
											} else
												Utils.println(System.err, "Is this an invalid qname: " + namespacekey
														+ " -> " + substring + subjectName, true, verbosemode);
										}
					...
					if (object.isResource()) {
						...
						if (objectShorthandNS == null || objectName == null) {
							...
							if (object.isAnon()) {
								...
								System.err
										.println("Encountered object URI without known namespace, treating as literal: "
												+ object);
		...
		try {
			...
			if (dropZoneFilesResponse.getExceptions() != null) {
				...
				if (dropZones.length > 0) {
					...
					Utils.println(System.out, "Found dropzone net address: " + targetDropzoneNetAddres, false,
							verbosemode);
					...
					try {
						...
						for (PhysicalFileStruct file : files) {
							Utils.println(System.out, "\t" + file.getName() + "-" + file.getFilesize(), true,
									verbosemode);
						}
					} catch (Exception e) {
						Utils.println(System.out, "Warning: could not fetch existing landingzone file list.", true,
								verbosemode);
					}
					success = true;
				}
