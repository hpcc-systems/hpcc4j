...
public class HPCCWsDFUClient {
	...
	/**
	 * @param logicalname - logical file to get file info for, can start with '~' or not
	 * @param clustername - Optional. If specified, the cluster on which to search for the file
		...
		try {
			...
			if (e != null) {
				for (org.hpccsystems.ws.client.soap.wsdfu.EspException espexception : e
					...
					Utils.println(
							System.out,
							"Error retrieving file type for file: "
									+ espexception.getSource()
	...
	/**
	 * @param logicalname - logical file to retrieve data for
	 * @param beginrow - beginning row to retrieve data from
	 * @param numrows - number of rows of data to retrieve
	 * @param clustername - Optional. If specified, the cluster on which to find the logical file.
	...
	/**
	 * @param logicalname
	 *            - logical filename to retrieve the dfu data columns for.
	 *            Currently this method/service call functions for THOR files
	 *            but will return nothing for CSV/XML/FLAT data files
	 * @param clustername
	 *            - optional. The cluster the logical filename is associated
	 *            with.
	 * @return ArrayList of DFUDataColumns
	 * @throws Exception
	 */
	public ArrayList<DFUDataColumn> getFileDataColumns(String logicalname,
		...
		try {
			...
			for (Method m : resp.getClass().getMethods()) {
				...
				if (m.getName().startsWith("getDFUDataNonKeyedColumns")
						&& m.getParameterTypes().length == 0) {
					final Object r = m.invoke(resp);
					DFUDataColumn[] thesecols = (DFUDataColumn[]) r;
					if (r != null) {
			...
			if (e != null) {
				for (org.hpccsystems.ws.client.soap.wsdfu.EspException espexception : e
					...
					Utils.println(
							System.out,
							"Error retrieving field names for file: "
									+ espexception.getSource()
									+ espexception.getMessage(), false, true);
				}
			}
	...
	public static String getOriginalWSDLURL() {
		return (new org.hpccsystems.ws.client.soap.wsdfu.WsDfuLocator())
				.getWsDfuServiceSoapAddress();
	}
	...
	public HPCCWsDFUClient(Connection baseConnection) {
		this(baseConnection.getProtocol(), baseConnection.getHost(),
				baseConnection.getPort(), baseConnection.getUserName(),
	 ...
	 */
	public HPCCWsDFUClient(String protocol, String targetHost,
			String targetPort, String user, String pass) {
	...
	/**
	 * Initializes the service's underlying soap proxy. Should only be used by
	 * constructors
	 *
	 * @param baseURL
	...
	private void initHPCCWsDFUSoapProxy(String baseURL, String user, String pass) {
		...
		if (wsDfuServiceSoapProxy != null) {
			WsDfuServiceSoap wsDfuServiceSoap = wsDfuServiceSoapProxy
					.getWsDfuServiceSoap();
	...
	/**
	 * This method retrieves a list of columns for a THOR or CSV dataset. -if
	 * THOR, this is a straightforward service call to get the columns. -if CSV,
	 * depending on whether or not the CSV was sprayed or output by thor, it may
	 * or may not - have a record definition associated with it. If it doesn't,
	 * the number of fields in the CSV - is calculated by retrieving the first
	 * line and splitting it on the separator, and returning - a list of string
	 * Field1, Field2, etc. fields (that match the standard CSV field naming
	 * conventions.)
	 * 
	 * @param datasetname
	 *            - the name of the dataset to get dataset fields for. Can begin
	 *            with '~' or not.
	 * @param clusterName
	 *            - optional. If given, the name of the cluster associated with
	 *            the dataset.
	 * @param fieldSeparator
	 *            - optional. if given, and if the file being retrieved is a csv
	 *            file, and if no field definitions exist for that csv,. this
	 *            separator will be used rather than the separator defined
	 *            during spraying when splitting up the first line of the file
	 *            to define the number of fields
	 * @return an ArrayList of DFUDataColumns containing the name and field
	 *         type.
	 * @throws Exception
	 */
	public ArrayList<DFUDataColumn> getDatasetFields(String datasetname,
			String clusterName, String fieldSeparator) throws Exception {
		String filetype = "";
			...
			if (filetype == null || filetype.equals("")) {
				...
				if (filetype == null || filetype.equals("")) {
					...
					if (resp.getFileDetail().getEcl() != null
							&& resp.getFileDetail().getEcl().toLowerCase()
									.contains("xpath")) {
						filetype = "XML";
						...
						throw new Exception(
								"Unknown file format for logical file "
										+ datasetname);
					}
				}
			}
		...
		if (filetype.equalsIgnoreCase("THOR")
				|| filetype.equalsIgnoreCase("FLAT")) {
			...
			try {
				fields = getFileDataColumns(datasetname, null);
				if (fields.size() == 2
						&& fields.get(0).getColumnLabel().equals("line")) {
	 ...
	 *            or in {STRING1 field} inline format,. currently handles XPATH
	 *            recordsets but doesn't save XPATH info. A few issues with
	 *            child dataset records
	 * @return An ArrayList of DFUDataColumns
	 * @throws Exception
	 */
	private ArrayList<DFUDataColumn> getRecordFromECL(String eclRecordDefinition)
			throws Exception {
		ArrayList<DFUDataColumn> cols = new ArrayList<DFUDataColumn>();
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (fieldargs.length < 2) {
				throw new Exception("Invalid record field definition "
						+ thisline);
			}
			if (!fieldargs[0]
					.toUpperCase()
